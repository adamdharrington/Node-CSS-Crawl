...
function siteFrameworks (site, callback){
  var siteFrameworks = {},
    ...
    p = site.pages.length;
  for (var i = 0; i<p;i+=1){
    if (site.pages[i].hasOwnProperty("frameworks")) {
      frameworkNames = Object.keys(site.pages[i].frameworks);
      break;
    }
  }
  frameworkNames.map(function(f){
    siteFrameworks[f] = 0;
    for (var i = 0; i<p;i+=1){
      // Take the largest framework match value
      if (site.pages[i].hasOwnProperty("frameworks") &&
        site.pages[i].frameworks[f] &&
        site.pages[i].frameworks[f] > siteFrameworks[f])
...
function checkFrameworks(site, callback){
  ...
  var fwks = site.frameworks,
    ...
    fwkQ = async.queue(function(task, callback){
      frameworks.record(site["host name"],
        site["page rank"],
        task.name,
...
function fwkToArray(fwks){
  ...
  return frameworks.map(function(f){return fwks[f] || 0}); // two decimal places
}
...
function totalUsed (site, callback){
  ...
  singleOccurances = unique.filter(function(o, i){
    return unique.indexOf(o) == i;
  });
...
function siteStats (site, callback){
  ...
  var m = function(prop){
    ...
    c ={
      ...
      "used selectorsU"           : m("used selectorsU")
    };
  ...
  */
  ...
  var report = [
    site["page count"],
    c["tot used sels"],
    1 - (c["tot used sels"]/ c.selectorsU.m),
    ...
    c["selector redundancyU"].m,
    c["selector redundancyU"].v,
    c["selector redundancyU"].sd,
